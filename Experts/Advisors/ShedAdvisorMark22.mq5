...
// Update order tracking array from actual positions - FIXED VERSION
void SyncOrderTracking() {
    ...
    // Build temporary array of current positions
    OrderInfo tempOrders[3];
    int tempCount = 0;
    
    for(int i = PositionsTotal() - 1; i >= 0 && tempCount < 3; i--) {
        ulong ticket = PositionGetTicket(i);
        if(ticket > 0) {
            if(PositionGetString(POSITION_SYMBOL) == _Symbol && 
               PositionGetInteger(POSITION_MAGIC) == MagicNumber) {
                
                // Check if this ticket exists in our tracking
                bool found = false;
                ...
                // If not found, it's a newly discovered position (shouldn't happen but be safe)
                if(!found) {
    ...
    // Copy temp array back to g_orders (compacted, no gaps)
    for(int i = 0; i < 3; i++) {
        if(i < tempCount) {
    ...
    // If all scaled orders closed, reset scaling direction
    if(g_orderCount == 0) {
        g_scalingDirection = 0;
    }
}
