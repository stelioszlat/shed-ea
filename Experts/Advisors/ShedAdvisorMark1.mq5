...
input double RiskPercent = 0.5;         // Risk per trade (not wired into triggers to preserve your behavior)
...
input string TradingSessionStart = "08:54";
...
double lastPullbackDownHigh = 0.0; // High on the last down candle in a buy-setup pullback sequence
double lastPullbackUpLow    = 0.0; // Low of the last up candle in a sell-setup pullback sequence
...
// Calculates the lot size given the risk percentage and the current adaptive ATR channel width (price units)
double CalculateLotSize(double channelWidthPriceUnits, double riskPercent=1.0) {
   ...
   PrintFormat("=== CalculateLotSize DEBUG ===");
   PrintFormat("Symbol: %s", _Symbol);
   PrintFormat("Channel Width (price units): %.6f", channelWidthPriceUnits);
   PrintFormat("Risk Percent: %.2f%%", riskPercent);
   PrintFormat("Account Equity: %.2f", equity);
   PrintFormat("Risk Money: %.2f", riskMoney);
   PrintFormat("SYMBOL_POINT: %.6f", point);
   PrintFormat("SYMBOL_TRADE_TICK_VALUE: %.6f", tickValue);
   PrintFormat("SYMBOL_VOLUME_STEP: %.2f", volumeStep);
...
// Efficiency ratio helper (not used directly below, retained for completeness)
...
bool TriggerSellOrder(double channelWidth, double riskPercent=0.1, int magicNumber=12345) {
   ...
   // NOTE: You had a constant 0.100 here. Keeping your original behavior.
...
// Function to modify stop loss of a position
bool ModifyStopLoss(ulong ticket, double TS) {
   ...
   request.sl = TS;
   request.tp = current_tp;  // Keep current TP
   
   // Send the modification request
   if(OrderSend(request, result)) {
      Print("[TRAILING_STOP] Stop Loss modified successfully for ticket: ", ticket, " New SL: ", TS);
      return true;
   } else {
...
void OnTick() {
   ...
   if (PositionSelect(_Symbol)) {
      ...
      double currHigh = iHigh(_Symbol, timeframe, 0);
      double currLow = iLow(_Symbol, timeframe, 0);
      ENUM_POSITION_TYPE orderType = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
      // SELL 
      if (orderType == POSITION_TYPE_BUY) {
         if ((entryPrice <= currClose + (0.6 * stopLoss)) && (entryPrice <= currLow + (0.3 * stopLoss))) {
            ModifyStopLoss(ticket, currLow - 0.02);
         }   
      } else if (orderType == POSITION_TYPE_SELL) {
         if ((entryPrice >= currClose - (0.6 * stopLoss)) && (entryPrice >= currHigh - (0.3 * stopLoss))) {
