...
input double BreakEvenMarginInPips = 0.01;
...
void checkTradeConditions() {
   ...
   if (currClose > upper && central20 > upper && currOpen > prevOpen && prevOpen < prevClose && lastPrevOpen > lastPrevClose) {
      ...
      if (!PositionSelect(_Symbol) && retracementPercent >= BreakoutRetracementLow && retracementPercent <= BreakoutRetracementHigh && orderPlacementToRetracement > OrderCloseToRetracementPercent && prevClose - prevOpen > 0.01) {
...
void checkBreakEvenConditions() {
   ...
   if (orderType == POSITION_TYPE_BUY) {
      ...
      if (entryPrice + (breakEven * stopLossDifference) <= currentPrice && !breakEvenRun) {
         double TS = entryPrice + BreakEvenMarginInPips;
...
void OnTick() {
   ...
   // TODO: Make Break Even calculate in terms of the small ATR width
