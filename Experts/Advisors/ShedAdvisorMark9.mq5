...
input double OrderCloseToRetracementPercent = 38.2;
...
void checkTradeConditions() {
   ...
   if (currClose > upper && central20 > upper && currOpen > prevOpen && prevOpen < prevClose && lastPrevOpen > lastPrevClose) {
      ...
      Print("ORDER PLACEMENT TO RETRACEMENT ", 100 - (retracementBuyHigh - currClose) / retracementSwing * 100, " %");
      if (!PositionSelect(_Symbol) && 100 - retracementPercent <= RetracementSwingPercent && (retracementBuyHigh - currClose) / retracementSwing * 100 < OrderCloseToRetracementPercent) {
   ...
   if (currClose < upper && currClose > lower) {
      state = TRADE_IDLE;
   }
}
