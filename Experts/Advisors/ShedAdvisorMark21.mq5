...
// --- Safety Limits ---
input int    MaxTotalOpenOrders = 3;                   // Absolute max orders open at once (1-3)
...
OrderInfo g_orders[3];  // Max 3 orders
int g_orderCount = 0;
...
void checkBreakEvenConditions() {
   ...
   // Process each order for break-even
   for(int i = 0; i < g_orderCount; i++) {
      if (g_orders[i].breakEvenReached) continue;  // Already at break-even
      
      ulong ticket = g_orders[i].ticket;
      if (!PositionSelectByTicket(ticket)) continue;
      
      double entryPrice = g_orders[i].entryPrice;
      double stopLoss = g_orders[i].initialSL;
      ...
      if (orderType == POSITION_TYPE_BUY) {
         ...
         if (entryPrice + (breakEven * stopLossDifference) <= currentPrice) {
            double TS = entryPrice + BreakEvenMarginInPips * GetPipSize();
            double currentSL = PositionGetDouble(POSITION_SL);
            
            if (TS > currentSL) {
               Print("[BREAK_EVEN_BUY #", i+1, "] Stop loss value:", TS);
               if (ModifyStopLoss(ticket, TS)) {
                  g_orders[i].breakEvenReached = true;
                  g_orders[i].currentSL = TS;
               }
            } else {
               Print("[BREAK_EVEN_BUY #", i+1, "] Skipped - current SL ", currentSL, " already better than ", TS);
               g_orders[i].breakEvenReached = true;
            }
         }
         ...
         if (entryPrice - (breakEven * stopLossDifference) >= currentPrice) {
            ...
            if (TS < currentSL) {
               Print("[BREAK_EVEN_SELL #", i+1, "] Stop loss value:", TS);
               if (ModifyStopLoss(ticket, TS)) {
                  g_orders[i].breakEvenReached = true;
                  g_orders[i].currentSL = TS;
               }
            } else {
               Print("[BREAK_EVEN_SELL #", i+1, "] Skipped - current SL ", currentSL, " already better than ", TS);
               g_orders[i].breakEvenReached = true;
            }
         }
      }
...
void checkRConditions() {
   ...
   // Process partial close for each order independently
   for(int i = 0; i < g_orderCount; i++) {
      ...
      // Skip if position volume is already very small (avoid closing tiny amounts)
      double minVolume = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN);
      if (currentVolume < minVolume * 2.0) continue; // need at least double min to close half
      
      // Calculate stop distance (R)
      double stopLossDistance = 0.0;
      if (orderType == POSITION_TYPE_BUY) {
         stopLossDistance = entryPrice - stopLoss; // how many price units we risk
      } else {
         stopLossDistance = stopLoss - entryPrice;
      }
      
      // If stopLossDistance is not positive, skip (invalid)
      if (stopLossDistance <= 0.0) continue;
      
      // Calculate the price target for the partial close:
      // For BUY: targetPrice = entryPrice + stopLossDistance * R1PartialPercent
      // For SELL: targetPrice = entryPrice - stopLossDistance * R1PartialPercent
      double targetPrice = 0.0;
      if (orderType == POSITION_TYPE_BUY) {
         targetPrice = entryPrice + stopLossDistance * R1PartialPercent;
      } else { // SELL
         targetPrice = entryPrice - stopLossDistance * R1PartialPercent;
      }
      ...
      // Check if we've reached the target and perform partial close (close half)
      if (orderType == POSITION_TYPE_BUY) {
         if (currentPrice >= targetPrice) {
            double lot = NormalizeVolume(_Symbol, currentVolume / 2.0);
            if (lot > 0.0 && TriggerPartialCloseOrder(lot)) {
               PrintFormat("[PARTIAL_CLOSE_BUY #%d] Closed %.2f lots at R=%.3f (targetPrice=%.5f, current=%.5f)", 
                           i+1, lot, R1PartialPercent, targetPrice, currentPrice);
         ...
         if (currentPrice <= targetPrice) {
            double lot = NormalizeVolume(_Symbol, currentVolume / 2.0);
            if (lot > 0.0 && TriggerPartialCloseOrder(lot)) {
               PrintFormat("[PARTIAL_CLOSE_SELL #%d] Closed %.2f lots at R=%.3f (targetPrice=%.5f, current=%.5f)", 
                           i+1, lot, R1PartialPercent, targetPrice, currentPrice);
               g_orders[i].partialCloseDone = true;
            }
         }
...
bool CanAddAnotherOrder(double currentPrice, ENUM_POSITION_TYPE orderType) {
   ...
   int minBars = (g_scalingDirection == -1) ? MinBarsBetweenLoserOrders : MinBarsBetweenWinnerOrders;
...
void SyncOrderTracking() {
   ...
   // If no positions, reset everything
   if (actualCount == 0) {
      g_orderCount = 0;
      g_scalingDirection = 0;
      g_lastOrderTime = 0;
   ...
   // Rebuild order array from actual positions
   int idx = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ulong ticket = PositionGetTicket(i);
      if(ticket > 0 && idx < 3) {
         if(PositionGetString(POSITION_SYMBOL) == _Symbol && 
            ...
            for(int j = 0; j < g_orderCount; j++) {
               if(g_orders[j].ticket == ticket) {
                  // Update current SL
                  g_orders[j].currentSL = PositionGetDouble(POSITION_SL);
                  found = true;
                  break;
               }
            }
   ...
   // Update count if positions were closed
   if (actualCount < g_orderCount) {
      g_orderCount = actualCount;
   }
}
