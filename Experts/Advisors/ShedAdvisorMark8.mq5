...
void checkTradeConditions() {
   ...
   if (currClose > upper && central20 > upper && currOpen > prevOpen && prevOpen < prevClose && state == RETRACEMENT_BUY) {
      ...
      if (!PositionSelect(_Symbol) && retracementPercent >= 70.0 && (100 - (retracementBuyHigh - currClose) / retracementSwing * 100) < 50.0) {
...
void findRetracementBuySwing() {
   ...
   while(breakoutBuyLow < previousLow) {
      breakoutBuyLow = previousLow;
      index++;
      previousLow = iLow(_Symbol, _Period, index + 1);
   }
