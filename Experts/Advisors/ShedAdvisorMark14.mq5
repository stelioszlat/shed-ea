...
void checkTradeConditions() {
   ...
   if (currClose > upper && central20 > upper && currOpen > prevOpen && prevOpen < prevClose && lastPrevOpen > lastPrevClose) {
      ...
      if (IsWithinSession() && !PositionSelect(_Symbol) && retracementPercent >= BreakoutRetracementLow && retracementPercent <= BreakoutRetracementHigh && orderPlacementToRetracement > OrderCloseToRetracementPercent) {
...
void checkTradesOffSession() {
   ...
   // TODO: consider closing orders with slippage deviation tolerance
   ...
   if (IsAfterOffSessionHour() /*&& positionProfit > 0.0*/) {
      double entryPrice = PositionGetDouble(POSITION_PRICE_OPEN);
      ENUM_POSITION_TYPE orderType = (ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE);
      double TS = 0.0;
      Print("[OFF SESSION] Checking position " + string(ticket) + " with entry price: ", entryPrice);
      
      switch (HandleOrdersOutOfSession) {
         case OffSessionCloseOrder:
            if (trade.PositionClose(ticket)) {
               Print("[CLOSE] Closing position " + string(ticket) + " off session");
            }
            break;
