...
input double PartialClosePercent = 1.0;
input int    Spread = 15;
input double RetracementSwingPercentUpper = 70.0;
input double RetracementSwingPercentLower = 10.0;
input double OrderCloseToRetracementPercent = 30.0;
input double DeepRetracementPercent = 80.0;
...
double CalculateLotSize(double stopLoss, double riskPercent=1.0) {
   ...
   double lotSize = 100.0 / valuePerLot;
...
double CalculateInitialStopLossWidth(double channelWidth, double channel20Width) {
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   long spread = Spread; // SymbolInfoInteger(_Symbol, SYMBOL_SPREAD);
...
bool TriggerSellOrder(double channelWidth, double channel20Width, double riskPercent=0.1, int magicNumber=12345) {
   ...
   double SL = NormalizeDouble(entryPrice + CalculateInitialStopLossWidth(channelWidth, channel20Width), _Digits) + InitialStopLossMarginInPips;
...
void checkTradeConditions() {
   ...
   if (currClose > upper && central20 > upper && currOpen > prevOpen && prevOpen < prevClose && lastPrevOpen > lastPrevClose) {
      ...
      if (!PositionSelect(_Symbol) && retracementPercent >= RetracementSwingPercentLower && retracementPercent <= RetracementSwingPercentUpper && orderPlacementToRetracement > OrderCloseToRetracementPercent && prevClose - prevOpen > 0.01) {
            // || (retracementPercent > DeepRetracementPercent && orderPlacementToRetracement >= OrderCloseToRetracementPercent))) {
   ...
   /*if (currClose < lower && central20 < lower && currOpen < prevOpen && prevOpen > prevClose && lastPrevOpen < lastPrevClose) {
      Print("BREAKOUT SELL POSSIBLE ORDER");
      ...
      double retracementPercent = retracementSwing / breakoutSwing * 100;
      Print("RETRACEMENT PERCENT ", retracementPercent, " %");
      
      double orderPlacementToRetracement = (retracementSellHigh - retracementSellLow) * 0.7;      
      Print("ORDER PLACEMENT TO RETRACEMENT ", orderPlacementToRetracement, " %");
      if (!PositionSelect(_Symbol) && 100 - retracementPercent <= RetracementSwingPercent && orderPlacementToRetracement < currClose) {
...
void findRetracementSellSwing() {
   ...
   breakoutSellHigh = retracementSellHigh;
   ...
   Print("BREAKOUT BUY:    LOW ", breakoutSellLow, ", HIGH ", breakoutSellHigh, ", DIFF ", breakoutSellHigh - breakoutSellLow);
   Print("RETRACEMENT BUY: LOW ", retracementSellLow, ", HIGH ", retracementSellHigh, ", DIFF ", retracementSellHigh- retracementSellLow);
}
...
void checkRConditions() {
   ...
   if (orderType == POSITION_TYPE_BUY) {
      ...
      if (stopLossDifference > 0 && currentPrice >= (stopLossDifference + entryPrice) * PartialClosePercent) {
      ...
      if (stopLossDifference > 0 && currentPrice <= (entryPrice - stopLossDifference) * PartialClosePercent) {
         double lot = NormalizeVolume(_Symbol, PositionGetDouble(POSITION_VOLUME) / 2.0);
         TriggerPartialCloseOrder(lot);
      }
   }
